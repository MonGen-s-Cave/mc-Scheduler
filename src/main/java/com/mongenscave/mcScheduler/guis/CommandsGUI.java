...
import static com.mongenscave.mcScheduler.util.CronUtil.getEventCommands;
...
public class CommandsGUI {
    ...
    public void openMenu(Player player, String eventName) {
        ...
        List<String> commands = getEventCommands(eventName);
        ...
        int totalPages = (int) Math.ceil((double) commands.size() / maxItemsPerPage);
        ...
        for (String command : commands) {
            gui.addItem(createCommandItem(command, eventName));
        }

        addDecorativeItems(gui);
        addCreateCommandButton(gui, eventName);
    ...
    private GuiItem createCommandItem(String command, String event) {
        ...
        return new GuiItem(itemStack, guiEvent -> {
            ...
            if(guiEvent.getClick() == ClickType.DROP) {
                ...
                guiEvent.getWhoClicked().sendMessage(ChatUtil.colorizeHex(configUtil.getConfig().getString("Settings.command_deleted")
                        .replace("%event_name%", event))
                        .replace("%command%", command));
            ...
            guiEvent.getWhoClicked().sendMessage(ChatUtil.colorizeHex(configUtil.getConfig().getString("Settings.command_rename")));
            playerEditCommand.put((Player) guiEvent.getWhoClicked(), new Time(command, event));
            guiEvent.getWhoClicked().closeInventory();
        });
    }

    private void addCreateCommandButton(PaginatedGui gui, String eventName) {
        ConfigUtil configUtil = Scheduler.getInstance().getConfigUtil();
        var guiConfig = configUtil.getGui("commands");
        ...
        GuiItem createButton = new GuiItem(createItem, event -> {
            ...
            playerCreateCommand.put((Player) event.getWhoClicked(), eventName);
            event.getWhoClicked().closeInventory();
            event.getWhoClicked().sendMessage(ChatUtil.colorizeHex(configUtil.getConfig().getString("Settings.command_create")));
        });
